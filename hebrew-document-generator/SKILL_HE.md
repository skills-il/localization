...
3. **סמנו `<w:rtl/>` רק על runs עבריים של פסקה ללא אותיות לטיניות** (תווית או כותרת עברית טהורה, ספרות מותרות). שם הדגל מעגן נקודתיים נגררות (`מחלות רקע:`) לקצה השמאלי ומספר כותרת מוביל (`2. כותרת`) לימין, בלעדיו Word מסיט את הפיסוק או המספר לקצה הלא נכון. הפיצול נשאר לפי סקריפט כדי שכל run יקבל את גופן הסקריפט המורכב הנכון.

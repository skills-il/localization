...
לא משתמשים ב-`unicode-bidi: bidi-override` לתוכן מעורב. הערך הזה מכבה את החלק המשתמע של אלגוריתם ה-bidi ומסדר כל תו בדיוק לפי ערך `direction`, כך שאנגלית בתוך override של RTL מוצגת עם אותיות הפוכות. שומרים אותו (או את `<bdo>`) למקרים שבהם רוצים לכפות בכוונה סדר חזותי ידוע.

בעיות bidi נפוצות:
- מספרי טלפון שהקבוצות בהם מופרדות ברווחים, או שמתחילים בקידומת בינלאומית עם סימן פלוס, מופיעים מבולגנים: עוטפים ב-`<bdo dir="ltr">` (דוגמאות בדוגמה 2). מספר עם מקפים בלבד כמו `050-321-4450` נשאר תקין גם בלי עזרה, כי מקף בודד בין שני מספרים מחבר אותם לרצף מספרי אחד.
...
עצבו את הערך ואז בודדו אותו: בידוד bidi רק מונע ממחרוזת *תקינה* להתהפך, הוא לא מייצר את המחרוזת הנכונה. השתמשו ב-Intl לעיצוב ואז בודדו: `Intl.NumberFormat('he-IL', { style: 'currency', currency: 'ILS' })` לסכומים בשקלים ו-`Intl.DateTimeFormat('he-IL')` לתאריכים, ועטפו את הפלט ב-`<bdi>` (או `unicode-bidi: isolate`) אם הוא יושב בתוך טקסט עברי. לא כופים `dir="ltr"` על פלט של `he-IL`: מחרוזת המטבע כוללת סימני כיוון מימין לשמאל משלה (U+200F) ומסודרת להקשר RTL, ולכן כפיית LTR מעבירה את ה-₪ לצד השני של המספר. ההבחנה: כופים `dir="ltr"` על מחרוזות ספרות גולמיות כמו מספרי טלפון, שאין בהן סימני כיוון, ורק מבודדים פלט של Intl, שיש בו. מפתחים נוטים לבלבל בין השניים ולהחיל תיקון bidi על באג עיצוב (ולהפך).
...
שדות טופס צריכים `dir="auto"`: הוסיפו `dir="auto"` לכל `<input>` ו-`<textarea>` כדי שכל ערך יפתור את כיוון הבסיס שלו. זה הבאג הכי בולט למשתמש קצה ב-RTL: אימייל או מילה באנגלית בתוך טופס עברי קופצים לצד הלא נכון בלעדיו. שימו לב שה-placeholder לא מפעיל זיהוי אוטומטי, אז קבעו את כיוון המנוחה ב-CSS אם המראה של שדה ריק חשוב. יש שני יוצאים מן הכלל. שדה `type="tel"` הוא כבר LTR כברירת מחדל (תקן HTML נותן לשדה טלפון בלי `dir` כיווניות LTR), ולכן לא מוסיפים לו `dir`. בשדות `email` ו-`url` רוב הערכים בלטינית, ולכן קובעים `dir="ltr"` בכוונה: עם `dir="auto"` ערך שמתחיל בעברית, למשל כתובת תחת סיומת הדומיין העברית "ישראל", הופך את השדה ל-RTL באמצע ההקלדה. כדאי לשקול את המחיר: `dir="ltr"` גם מיישר placeholder עברי לשמאל.
...
מצמידים `lang` ל-`dir` בקטעים משובצים. המאפיין `dir` מתקן רק את הסדר החזותי. מונח באנגלית בתוך טקסט עברי מסמנים כ-`<span lang="en" dir="ltr">`: מסנתזי דיבור ומתרגמי ברייל משתמשים בתג השפה כדי לעבור למצב השפה הנכון, וכללי `:lang()` לגופנים ולמיקוף נשענים עליו. בלעדיו אין לטכנולוגיה המסייעת שום סימן לצאת ממצב עברית בקטע הזה.
...
ריווח אותיות הוא כלי הדגשה בעברית, ולכן לא אוסרים אותו באופן גורף. טקסט רץ נשאר עם `letter-spacing: normal`, אבל הטיפוגרפיה העברית משתמשת בכוונה בריווח אותיות כדי להדגיש שמות, מונחים ומושגים. איפוס גורף כמו `letter-spacing: 0 !important` בערכת נושא RTL מוחק את ההדגשה הלגיטימית הזו.
...
**תמיכת RTL ב-Tailwind CSS (v4, עדכני; כלים לוגיים מאז v3.3):**
...
לחלק מהכלים אין גרסה לוגית והם נשארים פיזיים גם ב-v4: `translate-x-*` (מגירות ו-sheets שנכנסים מהצד), `origin-left` / `origin-right`, ו-`bg-linear-to-r` / `bg-linear-to-l`. לכל אחד מהם מצמידים דריסת `rtl:`, למשל `-translate-x-full rtl:translate-x-full` למגירה שנכנסת מצד ההתחלה.
...
ב-App Router של Next.js מעבירים את אותן אפשרויות ל-`AppRouterCacheProvider` של MUI (מתוך `@mui/material-nextjs/v16-appRouter`, לפי גרסת ה-major של Next.js) במקום לבנות מטמון נפרד: `<AppRouterCacheProvider options={{ key: 'muirtl', stylisPlugins: [prefixer, rtlPlugin] }}>`, ובתוכו `<ThemeProvider theme={theme}>`. ה-prop בשם `options` מועבר ישירות ל-`createCache` של Emotion.
...
6. טבלאות - העמודות מסתדרות מחדש אוטומטית, אבל הכריחו תאים של מספרים, קוד ותאריכים חזרה ל-LTR עם `<td dir="ltr">`, ויישרו אותם באופן עקבי (למשל `text-align: end`; יישור לבדו לא משנה כיוון)
...
10. פסי הגלילה יושבים משמאל ב-RTL - שמרו מקום עם `scrollbar-gutter: stable` (ב-Tailwind v4.3 ומעלה: `scrollbar-gutter-stable`) כדי למנוע קפיצת תוכן; `text-wrap: balance` משפר כותרות בעברית
...
- בדקו מחרוזת מעורבת אחת בכל משטח טקסט: `שלום John +972 50-123-4567 ₪1,234` מפעילה עברית, לטינית, מספר טלפון בינלאומי וסכום מטבע בבת אחת.
...
```html
<!-- שגוי: קבוצות מופרדות ברווח מוצגות בסדר הפוך (4450 321 050) -->
<p>התקשרו אלינו: 050 321 4450</p>

<!-- שגוי: הקידומת +972 קופצת לקצה השני (50-321-4450 972+) -->
...
מספר עם מקפים בלבד כמו `050-321-4450` מוצג נכון גם בלי עזרה, אז כדאי לבדוק את הפורמט בפועל לפני שמאשימים את ה-bidi. לפתרון מבוסס CSS בלבד משתמשים ב-`unicode-bidi: isolate` יחד עם `direction: ltr` על ה-span המכיל.
...
### קובצי עזר
- הקובץ `references/css-logical-properties.md` - טבלת מיפוי מלאה מתכונות CSS פיזיות ללוגיות (margin, padding, border, מיקום, יישור טקסט, גדלים) בתוספת המלצות למחסניות גופנים עבריים ל-sans-serif, serif ו-monospace. תסתכלו בו כשממירים גיליון סגנונות LTR לתכונות לוגיות תואמות RTL או בוחרים גופני ווב עבריים.
...
- מספרי טלפון, מספרי כרטיסי אשראי, קוד, נתיבי קבצים ופקודות CLI חייבים להישאר LTR גם בתוך מיכלים RTL. ערכים בתוך השורה עוטפים ב-`<bdo dir="ltr">`, ובערכת הנושא של ה-RTL מגדירים פעם אחת `code, pre, kbd, samp { direction: ltr; unicode-bidi: isolate; }` כדי שכל בלוק קוד וקיצור מקלדת יכוסו. סוכנים לפעמים נותנים להם לרשת RTL, וכך לוכסן בסוף נתיב או דגל של פקודה קופצים לקצה הלא נכון.
- במיכל גלילה RTL הערך `scrollLeft` הוא `0` בנקודת ההתחלה (הקצה הימני) ונעשה שלילי יותר ויותר לכיוון הסוף. קרוסלות מותאמות וכפתורי "הבא" שנכתבו כ-`el.scrollLeft += 300`, או שנחסמו עם `Math.max(0, ...)`, גוללים לכיוון הלא נכון או נתקעים בהתחלה. משתמשים ב-`el.scrollBy({ left: isRtl ? -300 : 300 })` ומתייחסים ל-`scrollLeft <= 0` כטווח הרגיל ב-RTL.
...
| MDN פסבדו-קלאס `:dir()` | https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/Selectors/:dir | תחביר, התנהגות מול סלקטורים על תכונת `[dir]` |
...
| Google Fonts עברית | https://fonts.google.com/?lang=he_Hebr | משפחות גופנים עבריים זמינות |
